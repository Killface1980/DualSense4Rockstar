...
namespace DualSense4GTAV
{
    ...
    private void OnTick(object sender, EventArgs e)
    {
      ...
      else if (playerped.IsInVehicle() || playerped.IsOnBike || playerped.IsInBoat || playerped.IsInHeli)
      {
        ...
        */
        ...
        //}
        else if (currentVehicle.Wheels != null)
        {
          if (!playerped.IsInBoat && !playerped.IsInHeli ||
               (playerped.IsInPlane && !currentVehicle.IsInAir))
          {
            if (currentVehicle.Wheels.Any(x => x.IsBursted))
            {
              int resistance = 4;
            ...
            //else if (currentVehicle.Wheels.Any(x => x.IsPunctured))
            //{
  ...
  else
  {
      ...
      else
      {
        ...
        else
        {
          if (playerWeapon != null)
          {
            ...
            int weaponStrength = 4 + (int)(weaponDamage / 6f);
      ...
      int fireRateAutomaticInt, List<int> triggerParameters, float timeBetweenShots)
    {
      ...
      if (Game.IsControlPressed(GTA.Control.Attack) || playerped.IsShooting)
      {
        if (lastAmmo != ammoInClip)
        {
          ...
          if (playerped.Weapons.Current.Hash == WeaponHash.Minigun)
            Script.Wait((int)(timeBetweenShots * 10000f));
          else
            Script.Wait((int)(timeBetweenShots * 1000f));
